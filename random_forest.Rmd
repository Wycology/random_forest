...
# Model building using `randomForest()` in R
...
Here I demonstrate how to use `randomForest()` in R to train a model and make prediction over new data. New data here actually has all features that were used in the training dataset. Generally, the two sets should be overlapping in values, though not exactly the same. By this I mean, it does not make sense to train a model then use it to make prediction on dataset with ranges beyond that of the training set.

We will use the inbuilt iris data for this demonstration. In that case, we use the four continuous variables (**`r names(iris[-5])`**) to predict **`r names(iris[5])`** of iris flower.
...
Here we break down the data into two sets with one being used for training the model and the other being used for testing the model performance. Normally training set should be having more data than the test set. But can vary depending on your use case.
...
So according to this, it seems Petal.Length and Petal.Width are the most important variables in predicting Species of iris. Let us plot these individually.
